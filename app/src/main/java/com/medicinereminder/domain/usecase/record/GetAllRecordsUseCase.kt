 ...
 * 用于从数据库获取所有服药记录列表
 */
class GetAllRecordsUseCase(private val medicineRecordRepository: MedicineRecordRepository) {

    /**
     * 执行获取所有服药记录操作
     * @return 服药记录模型列表的Flow
     */
    operator fun invoke(): Flow<List<MedicineRecordModel>> {
        return medicineRecordRepository.getAllRecords()
